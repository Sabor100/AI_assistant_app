...
// Exempel på system-instruktioner / "business info".
// Du kan justera texten efter behov.
const businessInfo = `
...
const API_KEY = "ENTER YOUR API KEY"; // Fyll i din Google Generative AI-nyckel
...
async function sendMessage() {
    ...
    if (userMessage.length) {
        try {
            ...
            // Visa en "loader" under tiden
            document.querySelector(".chat-window .chat").insertAdjacentHTML("beforeend",`
                <div class="loader"></div>
            `);

            // Skicka meddelande till modell
            const chat = model.startChat(messages);
            let result = await chat.sendMessageStream(userMessage);
            
            // Lägg till en tom .model-div för AI:ns svar
            document.querySelector(".chat-window .chat").insertAdjacentHTML("beforeend",`
                <div class="model">
                    <p></p>
                </div>
            ...
            messages.history.push({
                role: "model",
                parts: [{ 
                    text: modelMessages[modelMessages.length - 1].querySelector("p").innerHTML 
                }],
            });
...
// Koppla knapptryck till funktionen
document.querySelector(".chat-window .input-area button")
    .addEventListener("click", () => sendMessage());

// Öppna/stäng chattfönstret
